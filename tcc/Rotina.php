...
if($tarefa_enviar = "Enviar")
{
    if(strlen($tarefa_nome)>0 && strlen($tarefa_desc)>0 && strlen($tarefa_finaldata)>0)
    {
        ...
        if($comando->execute())
        {
          if($comando->rowCount()>0)
          {
              ...
              echo "<script>alert('Tarefa adicionada com sucesso!'); location= './Rotinax.php';</script>";
          }
    ...
    {
        echo "<script>alert('Uma ou mais areas ficaram em branco'); location= './Rotinax.php';</script>";
    }
